...
class QuoteDataService {
    ...
    @Published var firstTenQuote:[Quote] = []
    var quoteSubcription: AnyCancellable?
    
    var apiKey = "P3KNG49N6Q55AT60"
    
    init() {
        getQuotes(using: "AAPL")
    }

    
    func getQuotes(using symbol:String) {
        
        guard let url = URL(string: "https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=\(symbol)&apikey=\(apiKey)")  else {
            return
        }
        
        quoteSubcription = NetworkManager.download(url: url)
            ...
            .sink { (completion) in
                ...
                self.firstTenQuote.append(returnQuote)
                self.quoteSubcription?.cancel()
            }
    }
}
